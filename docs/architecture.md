...
Installer targets are thin destination adapters. They copy the same canonical folders and deliberately refuse overwrite. If an agent later requires format transformation rather than directory placement, add a tested adapter while preserving `skills/` as source of truth.
...
This is intentionally a Markdown-first repository with Python standard-library tooling. A package or compiled CLI can be introduced when distribution demand justifies its release and maintenance cost.
